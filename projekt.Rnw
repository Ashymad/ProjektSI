...
<<dates, fig.cap="Podpis">>=
library(ggplot2)
xseq <- seq(min(users$birth_date, na.rm=TRUE),
	    max(users$birth_date, na.rm=TRUE),
	    length=100)
scorn <- dnorm(as.numeric(xseq),
	       mean=mean(as.numeric(users$birth_date), na.rm=TRUE),
	       sd=sd(as.numeric(users$birth_date, na.rm=TRUE)))
hists <- hist(users$birth_date, plot=FALSE,
	      breaks='years')
