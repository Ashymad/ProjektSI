...
istotnej statystycznie zależności pomiędzy liczba obejrzanych seriali a średnią
oceną, natomiast znaleziono istotnie statystyczną zależność pomiędzy datą
urodzenia a średnią oceną, z czego wynikło że osoby młodsze w bardzo niewielkim
stopniu wystawiają wyższe oceny. Jako ostatnią wykonano analizę różnic pomiędzy
rozkładami dla każdej z płci, z czego wynikł brak zależności oceny od płci oraz
analizę różnic pomiędzy rozkładami dla każdego miasta Europy Centralnej, z której
wynikło istnienie istotnych statystycznie różnic pomiędzy ocenami w niektórych
państwach.
\section{Wstęp}
...
zaznaczona jest prosta regresji liniowej. Obliczono korelację pomiędzy danymi
uzyskując wartość \num{\Sexpr{round(bdate.cor$estimate,3)}}, co wskazuje na
bardzo nikłą zależność rosnącą. Przeprowadzono test t-Studenta sprawdzający
otrzymany wynik, otrzymując wartość $p$ wynoszącą
\num{\Sexpr{round(bdate.cor$p.value,2)}}. Ta wartość wskazuje na brak podstaw
do odrzucenia hipotezy o znaczeniu tej zależności, toteż można stwierdzić, że
młodsze osoby oceniają minimalnie wyżej niż starsze.
...
Następnie przetestowano uzyskano wynik, otrzymując wartość $p$ wynoszącą
\num{\Sexpr{round(shows.cor$p.value,3)}}, co oznacza że istnieją podstawy do
odrzucenia hipotezy o korelacji danych. Oznacza to że nie można powiedzieć, że
występuje zależność pomiędzy liczbą obejrzanych seriali a średnią z
wystawionych ocen.
...
natomiast różnice zbadano wykorzystując test Kruskala-Wallisa. Aby móc z niego
skorzystać jednak konieczne jest spełnienie warunku o jednorodności wariancji
danych, co sprawdza test Levene'a. Rysunek \ref{fig:sex_score} przedstawia
...
różnice pomiędzy Węgrami a Niemcami, oraz pomiędzy Węgrami i Polakami. Wynika z~%
